...
const AssignmentFilter = ({setSelect}:AssignmentFilterProps) => {
  return (
    <div className='border rounded flex flex-row items-center h-fit' >
        ...
        <select onChange={(e)=>setSelect(e.target.value)} defaultValue='All'  className="gap-3 h-full px-3 py-[0.3rem] text-[0.8rem] outline-none border dark:bg-[#0F1214]" name="assignment">
            <option className="dark:bg-[#0F1214] dark:text-slate-300" value="">All</option>
            <option className="dark:bg-[#0F1214] dark:text-slate-300" value="Pending">Unassigned</option>
            <option className="dark:bg-[#0F1214] dark:text-slate-300" value="Assigned">Assigned</option>
        </select>
