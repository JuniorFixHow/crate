...
const AssignmentFilter = ({setSelect}:AssignmentFilterProps) => {
  return (
    <div className='border rounded flex flex-row items-center' >
        ...
        <select onChange={(e)=>setSelect(e.target.value)} defaultValue='All'  className="gap-3 h-full px-3 text-[0.8rem] outline-none border-r border-r-slate-300 dark:bg-black" name="assignment">
            <option className="dark:bg-black dark:text-slate-300" value="All">All</option>
            <option className="dark:bg-black dark:text-slate-300" value="Pending">Unassigned</option>
            <option className="dark:bg-black dark:text-slate-300" value="Checked-in">Assigned</option>
        </select>
        <div onClick={()=>setSelect('All Zones')}   className="flex flex-row cursor-pointer gap-3 dark:bg-transparent dark:hover:border-blue-600 hover:bg-slate-200 px-3 py-1 ">
