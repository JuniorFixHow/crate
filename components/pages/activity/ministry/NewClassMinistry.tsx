...
const NewClassMinistry = ({inputRef, newMode, setNewMode, currentClassministry}:NewClassMinistryProps) => {
    const [loading, setLoading] = useState<boolean>(false);
    const [title, setTitle] = useState<string>('');
    const {user} = useAuth();
    const {refetch} = useFetchClassministry();
            ...
            <div className="flex border w-80 flex-col p-5 gap-6 bg-white dark:bg-transparent dark:border shadow-md rounded-md">
                ...
                <form onSubmit={currentClassministry ? handleUpdateMinistry:handleNewMinistry} ref={formRef}  className="flex flex-col gap-5">
                    ...
                    <div className="flex justify-end items-center gap-3">
                        ...
                        <AddButton className="rounded" disabled={loading} text={loading? 'loading' : currentClassministry? 'Update' : "Proceed"} noIcon smallText />
                    </div>
                </form>
            </div>
        </Slide>
