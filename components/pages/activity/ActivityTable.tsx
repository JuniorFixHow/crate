...
const ActivityTable = () => {
  ...
  const {activities, loading} = useFetchActivities();
  // const [deleteMode, setDeleteMode] = useState<boolean>(false);
  ...
  const handleDeleteActivity = async()=>{
    try {
      ...
      console.log(error);
      setResponse({message:'Error occured deleting the activity', error:true});
    }
  }
    ...
    <div className="flex w-full">
        {
            ...
            <Paper className='w-full' sx={{ height: 480, }}>
                <DataGrid
                    rows={SearchActivityWithChurch(activities, search, churchId)}
