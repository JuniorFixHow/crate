...
export type SingleRelationshipInfoModalProps = {
    infoMode:boolean,
...
const SingleRelationshipInfoModal = ({infoMode, currentMember, noChurch, setInfoMode, currentRelationship, setCurrentRelationship}:SingleRelationshipInfoModalProps) => {
    const church = currentRelationship?.churchId as IChurch;
    const members = currentRelationship?.members as IMember[];

    const cMember = members?.filter((member)=>member?._id === currentMember?._id)[0]
    const {user} = useAuth();
        ...
        <div className="flex flex-col min-w-72 h-full bg-white dark:bg-black dark:border rounded-l-lg p-4 overflow-y-scroll scrollbar-custom">
            ...
            <div className="flex flex-col dark:text-slate-200">
                ...
                <div className="flex flex-col dark:text-slate-200">
                    ...
                    <Link href={`${cMember?._id}`} className='table-link' >{cMember?.name}</Link>
                </div>
                <div className="flex flex-col dark:text-slate-200">
                    <span className='text-[1.1rem] font-semibold text-slate-700' >Relations</span>
                    {
                        members?.
                        filter((item)=>item?._id !== cMember?._id)
                        .map((member)=>(
                            <>
                            {
                                showMember ?
...
export default SingleRelationshipInfoModal