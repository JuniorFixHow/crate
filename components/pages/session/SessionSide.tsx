    ...
    <div className='flex  flex-col w-52 rounded gap-5 py-4 px-2 bg-white border dark:bg-black' >
      ...
      </div>
      {
        ...
        <div onClick={()=>handleClickSession(session)} key={session?._id}    className="flex flex-row items-center relative">
            ...
            <div   className={`flex relative w-full bg-[#F4F4F4] ${currentSession?._id === session?._id && 'border-red-700 dark:border-blue-700'} dark:bg-black p-1 border cursor-pointer  rounded flex-row justify-between items-center ${className}`} >
