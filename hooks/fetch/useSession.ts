import { getChurchSessions, getEventSessions, getEventSessionsForChildren, getPublicSessions, getSessions, getUserSessionsWithoutEvent } from "@/lib/actions/session.action";
        ...
        try {
            ...
            if(id){
               evts = await getUserSessionsWithoutEvent(id)
                ...
                evts = isAdmin ? await getSessions(type) : (!isAdmin && orgReader) ? await getPublicSessions(type) : await getChurchSessions(type, user?.churchId);
            }
...
export const useFetchSessionsWithEventForChildren = (eventId:string)=>{
    const searchParams = useSearchParams();
        ...
        try {
            ...
            if(id){
               evts = await getUserSessionsWithoutEvent(id)
                ...
                evts = await getEventSessionsForChildren(eventId);
            }
            const sorted =  evts.sort((a, b)=> new Date(a.createdAt!)<new Date(b.createdAt!) ? 1:-1);
            return sorted;
        } catch (error) {
