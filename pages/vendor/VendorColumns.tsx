...
export const VendorsColumns = (
    handleInfo:(data:VendorProps)=>void,
    handleDelete:(data:VendorProps)=>void,
    handleNew:(data:VendorProps)=>void,
) => [
    {
        field:'name',
        headerName:'Vendor',
        ...
        renderCell:(params:GridRenderCellParams)=>{
            return(
                <div className="flex gap-1 items-center justify-center">
                    <Link href={{pathname:'/dashboard/churches', query:{id:2}}}    className="table-link" >{params?.row?.church}</Link>
                </div>
            )
        }
    },
    {
        ...
        renderCell:(params:GridRenderCellParams)=>{
            return(
                <div className="flex gap-1 items-center justify-center">
                    <Link href={{pathname:'/dashboard/members', query:{registeredBy:params?.row?.id}}}    className="table-link text-center" >{params?.row?.registrants}</Link>
                </div>
            )
        }
    },
